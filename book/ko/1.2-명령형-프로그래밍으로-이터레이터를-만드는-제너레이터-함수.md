...
이 경우 `iter.next()`를 처음 호출하면 `1`이 반환되고, 함수는 `console.log('hi');`의 이전 지점에서 일시 중지됩니다. 두 번째로 `iter.next()`를 호출하면 `'hi'`가 출력되고, `2`가 반환됩니다. 이렇게 `done` 속성이 `true`가 될 때까지 이터레이터는 `yield` 키워드가 있는 지점까지 실행을 계속하고, 값을 반환하며 일시 중지됩니다.
