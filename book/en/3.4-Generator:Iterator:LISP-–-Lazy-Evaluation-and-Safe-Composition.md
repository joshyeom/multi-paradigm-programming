...
When using the array `concat` method, creating a large combined array increases memory usage. This occurs because the newly created array allocates a large index table to hold all elements, and every slot must be reassigned. In contrast, generator-based `concat` does not copy values; it only generates them when needed, making it more efficient. In [Code 3-58], even though the sole purpose is to compute the `acc` value, a new array `arr2` is still created. Meanwhile, in the generator-based `concat` example, no array copying is involved, and only the necessary operations are performed to produce `acc2`.
